...
export class DashboarduserComponent implements OnInit {
  ...
  constructor(
    ...
    this.authService.isLoggedin.subscribe(isLoggedin => {
      ...
      if(this.isLoggedin) {
        this.httpClient.get<User>('http://213.165.74.6:9090/users/' + this.userId)
          .subscribe(user => {
            this.user = user;
            if (user) {
  ...
  ngOnInit(): void {
    ...
    if (this.userId) {
      ...
      const eventosUrl = `http://213.165.74.6:9090/eventos/filter-by-user/${this.userId}`;
  ...
  save() {
    ...
    if (this.isUpdate) {
      this.httpClient.put<Evento>('http://213.165.74.6:9090/eventos/' + this.evento?.id, formData)
        .subscribe(
          response => {
      ...
      this.httpClient.post<Evento>('http://213.165.74.6:9090/eventos', formData)
        .subscribe(
          response => {
  ...
  onDrop(event: CdkDragDrop<Evento[]>): void {
    ...
    this.httpClient.delete(`http://213.165.74.6:9090/eventos/${eventoId}`).subscribe({
      next: () => {
  ...
  eliminarEventoBackend(id: number): void {
    this.httpClient.delete(`http://213.165.74.6:9090/eventos/${id}`).subscribe({
